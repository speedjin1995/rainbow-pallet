...
<?php
    if (!hasModulePermission('Master Data', 'PV Items', ['view'])){
...
<head>
    <title><?=$languageArray['pv_items_code'][$language]?> | Synctronix - Weighing System</title>
                                                                    ...
                                                                    <div class="card-body">
                                                                        <div class="row">
                                                                            <div class="col-xxl-12 col-lg-12 mb-3">
                                                                                <div class="row">
                                                                                    <label for="itemCode" class="col-sm-4 col-form-label"><?=$languageArray['item_code_code'][$language]?></label>
                                                                                    <div class="col-sm-8">
                                                                                        <input type="text" class="form-control" id="itemCode" name="itemCode" placeholder="<?=$languageArray['item_code_code'][$language]?>" required>
                                                                            ...
                                                                            <div class="col-xxl-12 col-lg-12 mb-3">
                                                                                <div class="row">
                                                                                    <label for="itemName" class="col-sm-4 col-form-label"><?=$languageArray['item_name_code'][$language]?></label>
                                                                                    <div class="col-sm-8">
                                                                                        <input type="text" class="form-control" id="itemName" name="itemName" placeholder="<?=$languageArray['item_name_code'][$language]?>" required>
                                                                                        <div class="invalid-feedback">
                                                                                            <?=$languageArray['please_fill_in_the_field_code'][$language]?>
                                                                                        </div>
                                                                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div class="card">
                                                    <div class="card-header">
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            <div class="flex-shrink-0">
                                                                <?php if(hasModulePermission('Master Data', 'PV Items', ['download_template'])): ?>
                                                                <a href="template/Pv_Item_Template.xlsx" download>
                                                                ...
                                                                <?php if(hasModulePermission('Master Data', 'PV Items', ['upload_excel'])): ?>
                                                                ...
                                                                <?php if(hasModulePermission('Master Data', 'PV Items', ['cancelled'])): ?>
                                                                ...
                                                                <?php if(hasModulePermission('Master Data', 'PV Items', ['create'])): ?>
                                                                <button type="button" id="addPvItem" class="btn btn-success waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#addModal">
                                                    ...
                                                    <div class="card-body">
                                                        <table id="pvItemTable" class="table table-bordered nowrap table-striped align-middle" style="width:100%">
                                                            <thead>
                                                                <tr>
...
$(function () {
    $('#selectAllCheckbox').on('change', function() {
        var checkboxes = $('#pvItemTable tbody input[type="checkbox"]');
    ...
    table = $("#pvItemTable").DataTable({
        ...
        'serverMethod': 'post',
        'ajax': {
            'url':'php/modules/pvItems/loadPvItems.php'
        },
        'columns': [
            {
            ...
            { 
                ...
                render: function ( data, type, row ) {
                    if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['PV Items'] && ['edit', 'cancelled'].some(p => permissions['Master Data']['PV Items'].includes(p)))) {
                        ...
                        if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['PV Items'] && permissions['Master Data']['PV Items'].includes('edit'))) {
                            buttons += `
                                    <li>
                        ...
                        if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['PV Items'] && permissions['Master Data']['PV Items'].includes('cancelled'))) {
                            buttons += `
                                    <li>
    ...
    $('#submitRawMat').on('click', function(){
        ...
        if($('#rawMatForm').valid()){
            $('#spinnerLoading').show();
            $.post('php/modules/pvItems/pvItem.php', $('#rawMatForm').serialize(), function(data){
    ...
    $('#submitWeights').on('click', function(){
        ...
        $.ajax({
            url: 'php/modules/pvItems/uploadPvItems.php',
            type: 'POST',
            ...
            success: function(response) {
                ...
                if (obj.status === 'success') {
                    ...
                    toastr["success"](obj.message, "Success:");
                    $('#pvItemTable').DataTable().ajax.reload(null, false);
                } 
                else if (obj.status === 'failed') {
                    $('#spinnerLoading').hide();
                    toastr["error"](obj.message, "Failed:");
    ...
    $('#addPvItem').on('click', function(){
        $('#addModal').find('#id').val("");
        $('#addModal').find('#itemCode').val("");
        $('#addModal').find('#itemName').val("");
    ...
    $('#multiDeactivate').on('click', function () {
        ...
        $("#pvItemTable tbody input[type='checkbox']").each(function () {
            if (this.checked) {
        ...
        if (selectedIds.length > 0) {
            if (confirm('Are you sure you want to delete these pv item?')) {
                $.post('php/modules/pvItems/deletePvItem.php', {userID: selectedIds, type: 'MULTI'}, function(data){
                    var obj = JSON.parse(data);
            ...
            alert("Please select at least one pv item to delete.");
            $('#spinnerLoading').hide();
        }     
    });
});
...
function edit(id){
    $('#spinnerLoading').show();
    $.post('php/modules/pvItems/getPvItem.php', {userID: id}, function(data)
    {
        var obj = JSON.parse(data);
        if(obj.status === 'success'){
            $('#addModal').find('#id').val(obj.message.id);
            $('#addModal').find('#itemCode').val(obj.message.item_code);
            $('#addModal').find('#itemName').val(obj.message.item_name);
...
function deactivate(id){
    ...
    if (confirm('Are you sure you want to delete this raw material?')) {
        $.post('php/modules/pvItems/deletePvItem.php', {userID: id}, function(data){
            var obj = JSON.parse(data);
