...
if (isset($_POST['userID'])) {
    ...
    if ($type == 'MULTI') {
        ...
        if ($stmt = $db->prepare("UPDATE Sawn_Timber_Species SET status=?, modified_by=? WHERE id IN ($ids)")) {
        ...
        if ($stmt = $db->prepare("UPDATE Sawn_Timber_Species SET status=?, modified_by=? WHERE id=?")) {
